"""
Training script for Whisper-based hybrid contrastive learning (genre + BPM).

Uses HybridJaccardBPMLoss which weights positive pairs by a convex combination of
genre-match and BPM Gaussian similarity.
"""
...
class HybridContrastiveTrainer:
    """Trainer for hybrid (genre + BPM) supervised contrastive learning."""
    ...
    def train_epoch(self):
        ...
        for batch_idx, (audio, genre_labels, bpm_values) in enumerate(pbar):
            audio = audio.to(self.device)
            genre_labels = genre_labels.to(self.device)
            bpm_values = bpm_values.to(self.device)

            embeddings = self.model(audio)
            loss = self.criterion(embeddings, genre_labels, bpm_values)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for audio, genre_labels, bpm_values in tqdm(self.val_loader, desc='Validation'):
                audio = audio.to(self.device)
                genre_labels = genre_labels.to(self.device)
                bpm_values = bpm_values.to(self.device)
    ...
    def train(self):
        print("\n" + "=" * 60)
        print("Starting Hybrid (Genre + BPM) Contrastive Training")
        print("=" * 60)
        print(f"Device: {self.device}")
        print(f"Epochs: {self.num_epochs}")
        print(f"Alpha (genre weight): {self.criterion.alpha}")
        print(f"BPM sigma: {self.criterion.bpm_sigma}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train Whisper Contrastive Model (Hybrid: Genre + BPM)'
    )
    ...
    parser.add_argument('--alpha', type=float,
                        default=config.CONTRASTIVE_TRAINING.get('alpha', 0.7),
                        help='Weight for genre match (1-alpha goes to BPM)')
    parser.add_argument('--bpm-sigma', type=float,
                        default=config.CONTRASTIVE_TRAINING.get('bpm_sigma', 10.0),
                        help='Gaussian kernel width for BPM similarity')
    parser.add_argument('--bpm-cache', type=str, default=None,
                        help='Path to pre-computed BPM cache JSON')
    ...
    if args.checkpoint_dir is None:
        ...
        args.checkpoint_dir = os.path.join(
            ...
            f'whisper_contrastive_hybrid_{timestamp}',
        )
    ...
    print("Creating dataloaders (hybrid: genre + BPM)...")
